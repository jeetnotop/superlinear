"""Precomputed sliding-window span attention kernels for GQA.

This is the GQA companion to [superlinear/span/_triton_precomputed_sw.py](superlinear/span/_triton_precomputed_sw.py).
It fuses span attention with a precomputed sliding-window output + LSE coming from
FlexAttention or the Triton sliding-window path.

Internal module: used by attention orchestration in [superlinear/attention/_prefill.py](superlinear/attention/_prefill.py).
"""
...
from superlinear.kernels.common.power import derive_stripe_power_params, window_len_from_sw_index
from ._triton_impl import _assert_no_span_sw_overlap, _next_power_of_two
from ._triton_precomputed_sw import fused_span_attention_with_precomputed_sw


@triton.jit
def fused_span_forward_precomputed_sw_kernel_gqa(
    Q_ptr,
    K_ptr,
    V_ptr,
    qstart_ptr,
    ...
    sw_offset = (batch_idx * H_Q + q_head_idx) * L_Q + q_idx
...
@triton.jit
def fused_span_backward_precomputed_sw_kernel_gqa(
    Q_ptr,
    K_ptr,
    V_ptr,
    dOut_ptr,
...
def fused_span_triton_precomputed_sw_gqa(
    Q2: torch.Tensor,
    K: torch.Tensor,
    V: torch.Tensor,
    qstart: torch.Tensor,
    ...
    _assert_no_span_sw_overlap(qendc, cachec.view(1, 1, -1, 1), sw_index, L_KV, window_len=window_len)

    out = torch.empty((B, H_q, L_Q, num_spans, D), device=Q2.device, dtype=Q2.dtype)
    ...
    fused_span_forward_precomputed_sw_kernel_gqa[grid](
        Q2c,
        Kc,
        Vc,
        qstartc,
...
class FusedSpanPrecomputedSWGQAFn(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        Q2,
        ...
        kv_repeat = H_q // H_kv
        out = fused_span_triton_precomputed_sw_gqa(
            Q2,
            K,
            V,
            qstart,
    ...
    def backward(ctx, grad_out):
        ...
        grid = (B * H_q * L_Q * num_spans,)
        fused_span_backward_precomputed_sw_kernel_gqa[grid](
            Q2c,
            Kc,
            Vc,
            grad_out_c,
...
def fused_span_attention_with_precomputed_sw_gqa(
    Q2: torch.Tensor,
    K: torch.Tensor,
    V: torch.Tensor,
    qstart: torch.Tensor,
    ...
    if not Q2.is_cuda:
        params = derive_stripe_power_params(search_power=search_power, inv_search_power_int=inv_search_power_int)
        if params.triton_inv_n != 2:
            raise NotImplementedError(
                "Non-CUDA fused_span_attention_with_precomputed_sw_gqa only supports p=0.5 for now "
        ...
        return fused_span_attention_with_precomputed_sw(
            Q2,
            K_rep,
            V_rep,
            qstart,
    ...
    return FusedSpanPrecomputedSWGQAFn.apply(
        Q2,
        K,
        V,
        qstart,
